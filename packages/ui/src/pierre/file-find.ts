...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("merge-find")
  api.delete("merge-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("merge-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("merge-find", new Highlight(...rest))
    return true
  }
