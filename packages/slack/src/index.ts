...
import { createOpencode, type ToolPart } from "@merge-ai/sdk"
...
console.log("🚀 Starting merge server...")
const merge = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await merge.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new merge session...")
    const { client, server } = merge
    ...
    console.log("✅ Created merge session:", createResult.data.id)
  ...
  console.log("📝 Sending to merge:", message.text)
