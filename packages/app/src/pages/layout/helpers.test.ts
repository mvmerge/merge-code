...
import { type Session } from "@merge-ai/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("merge://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("merge://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("merge://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("merge://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("merge://open-project")).toBeUndefined()
    expect(parseDeepLink("merge://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "merge://open-project?directory=/a",
      "merge://other?directory=/b",
      "merge://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("merge://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("merge://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("merge://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("merge://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "merge://new-session?directory=/a",
      "merge://open-project?directory=/b",
      "merge://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MERGE__: {
        deepLinks: ["merge://open-project?directory=/a"],
      },
    } as unknown as Window & { __MERGE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["merge://open-project?directory=/a"])
