...
const mergeDir = path.join(repoDir, "packages", "merge")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "merge-e2e-"))
const keepSandbox = process.env.MERGE_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  MERGE_DISABLE_SHARE: process.env.MERGE_DISABLE_SHARE ?? "true",
  MERGE_DISABLE_LSP_DOWNLOAD: "true",
  MERGE_DISABLE_DEFAULT_PLUGINS: "true",
  MERGE_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  MERGE_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  MERGE_E2E_PROJECT_DIR: repoDir,
  MERGE_E2E_SESSION_TITLE: "E2E Session",
  MERGE_E2E_MESSAGE: "Seeded for UI e2e",
  MERGE_E2E_MODEL: "merge/gpt-5-nano",
  MERGE_CLIENT: "app",
  MERGE_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_MERGE_SERVER_HOST: "127.0.0.1",
  VITE_MERGE_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../merge/src/util/log")
    const install = await import("../../merge/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../merge/src/server/server")
    inst = await import("../../merge/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`merge server listening on http://127.0.0.1:${serverPort}`)
