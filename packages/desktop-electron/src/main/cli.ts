...
const CLI_INSTALL_DIR = ".merge/bin"
const CLI_BINARY_NAME = "merge"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `merge-cli${suffix}`)
    : join(root, "../../resources", `merge-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "merge-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    MERGE_SERVER_USERNAME: "merge",
    MERGE_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    MERGE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    MERGE_EXPERIMENTAL_FILEWATCHER: "true",
    MERGE_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.merge/bin/merge"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://merge.ai/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
