...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.merge.desktop.dev",
  beta: "ai.merge.desktop.beta",
  prod: "ai.merge.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.merge.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `merge.settings.dat` is special: it maps to the `merge.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"merge.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // merge.settings.dat → the electron settings store ("merge.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "merge.global.dat").
  const storeName = filename === "merge.settings.dat" ? "merge.settings" : filename
