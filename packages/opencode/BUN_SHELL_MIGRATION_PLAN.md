...
Replace runtime Bun shell template-tag usage in `packages/merge/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/merge/src` is removed except:
