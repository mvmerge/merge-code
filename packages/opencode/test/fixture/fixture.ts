...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(path.join(os.tmpdir(), "merge-test-" + Math.random().toString(36).slice(2)))
  await fs.mkdir(dirpath, { recursive: true })
  if (options?.git) {
    await $`git init`.cwd(dirpath).quiet()
    await $`git config core.fsmonitor false`.cwd(dirpath).quiet()
    await $`git config user.email "test@merge.test"`.cwd(dirpath).quiet()
...
export function tmpdirScoped(options?: { git?: boolean; config?: Partial<Config.Info> }) {
  return Effect.gen(function* () {
    ...
    const dir = yield* fs.makeTempDirectoryScoped({ prefix: "merge-test-" })
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@merge.test")
    ...
    if (options?.config) {
      yield* fs.writeFileString(
        ...
        JSON.stringify({ $schema: "https://merge.ai/config.json", ...options.config }),
      )
    }
