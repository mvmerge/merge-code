...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MERGE_TEST ?? "")'), {
      env: {
        MERGE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MERGE_TEST_SHELL"], {
      shell: true,
      env: {
        MERGE_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MERGE_TEST_SHELL=ok")
  })
