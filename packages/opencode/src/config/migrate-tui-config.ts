...
const TUI_SCHEMA_URL = "https://merge.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from merge.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const merge = await mergeFiles(input)
...
async function mergeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.MERGE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("merge", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "merge")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "merge"))
  }
  if (Flag.MERGE_CONFIG) files.push(Flag.MERGE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "merge"))
