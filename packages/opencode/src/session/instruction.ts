...
function globalFiles() {
  ...
  if (Flag.MERGE_CONFIG_DIR) {
    files.push(path.join(Flag.MERGE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.MERGE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.MERGE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no MERGE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.MERGE_CONFIG_DIR, Flag.MERGE_CONFIG_DIR).catch(() => [])
}
