...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run merge",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for merge session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found merge session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["merge", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting merge...")
        UI.println()

        const mergeArgs = sessionId ? ["-s", sessionId] : []
        const mergeProcess = Process.spawn(["merge", ...mergeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await mergeProcess.exited
        if (code !== 0) throw new Error(`merge exited with code ${code}`)
      },
    })
  },
})
