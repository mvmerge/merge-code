...
import type { AssistantMessage, Event, MergeClient, SessionMessageResponse, ToolPart } from "@merge-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: MergeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `merge auth login` in the terminal",
        name: "Login with merge",
        id: "merge-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Merge stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const mergeProvider = providers.find((p) => p.id === "merge")
    if (mergeProvider) {
      if (mergeProvider.models["big-pickle"]) {
        return { providerID: ProviderID.merge, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(mergeProvider.models))
      if (best) {
        return {
