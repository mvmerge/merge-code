...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for merge.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal merge sessions
...
```bash
MERGE_ENABLE_QUESTION_TOOL=1 merge acp
```
...
- **Terminal Integration**: Full terminal support via merge's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | merge acp
```
...
ACP sessions map cleanly to merge's internal session model:
