...
**Only** add if it's specifically about the Desktop application or `merge web` view. **Do not** add for terminal, TUI, or general merge issues.
...
**Only** add if the issue mentions "zen" or "merge zen" or "merge black".

If the issue doesn't have "zen" or "merge black" in it then don't add zen label

#### core

Use for core server issues in `packages/merge/`, excluding `packages/merge/src/cli/cmd/tui/`.
...
TUI (`packages/merge/src/cli/cmd/tui/...`):
...
Core (`packages/merge/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for merge server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
